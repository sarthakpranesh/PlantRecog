...
import type { PredictPayload, ServiceAvailablePayload } from "../types";
...
const FetchBuilder = (route: string, conf: RequestInit | undefined) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log(`Service->PlantRecog->Endpoint: ${baseUrl + route}\tMethod: ${conf?.method ?? 'GET'}\tBody: ${conf?.body ? JSON.stringify(conf.body) : 'None'}`);
      const rawResp = await fetch(baseUrl + route, conf);
      const resp = await rawResp.json();
      console.log(`Service->PlantRecog->${route} response:`, resp.message);
      resolve(resp);
    } catch (err: any) {
      console.log(`Service->PlantRecog->${route} error:`, err.message, err);
